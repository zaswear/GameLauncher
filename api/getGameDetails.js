// Contenido para /api/getGameDetails.js

export default async function handler(request, response) {
    const RAWG_API_KEY = process.env.RAWG_API_KEY;

    if (!RAWG_API_KEY) {
        return response.status(500).json({ message: "La clave de API de RAWG no está configurada." });
    }

    try {
        // Extraemos el ID del juego que nos pide el frontend desde la URL
        const urlParams = new URL(request.url, `https://${request.headers.host}`).searchParams;
        const gameId = urlParams.get('id');

        if (!gameId) {
            return response.status(400).json({ message: "No se proporcionó un ID de juego." });
        }

        // Hacemos la petición a la API de RAWG para obtener los detalles de ESE juego en específico
        const url = `https://api.rawg.io/api/games/${gameId}?key=${RAWG_API_KEY}`;
        
        const apiResponse = await fetch(url);
        if (!apiResponse.ok) {
            throw new Error(`Error al obtener detalles de RAWG. Estado: ${apiResponse.status}`);
        }

        const gameDetails = await apiResponse.json();

        // Enviamos la respuesta completa al frontend
        response.setHeader('Cache-Control', 's-maxage=86400, stale-while-revalidate'); // Cache por 1 día
        response.status(200).json(gameDetails);

    } catch (error) {
        console.error("ERROR EN LA FUNCIÓN getGameDetails:", error.message);
        response.status(500).json({ 
            message: "Error interno del servidor al obtener detalles del juego.",
            details: error.message 
        });
    }
}